...
# ---- HANDLE SCANNER (NtQuerySystemInformation) -------------------------------
# Identifies processes holding directory handles — covers cases Restart Manager can't.
Add-Type -TypeDefinition @'
using System;
...
public static class HandleScanner {
    [DllImport("ntdll.dll")] static extern int NtQuerySystemInformation(int cls, IntPtr buf, int size, out int needed);
    [DllImport("ntdll.dll")] static extern int NtQueryObject(IntPtr h, int cls, IntPtr buf, int size, out int needed);
    [DllImport("kernel32.dll")] static extern IntPtr OpenProcess(int access, bool inherit, int pid);
    [DllImport("kernel32.dll")] static extern bool DuplicateHandle(IntPtr src, IntPtr srcH, IntPtr dst, out IntPtr dstH, int access, bool inherit, int opts);
    [DllImport("kernel32.dll")] static extern bool CloseHandle(IntPtr h);
    [DllImport("kernel32.dll")] static extern IntPtr GetCurrentProcess();
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)]
    static extern int QueryDosDevice(string deviceName, IntPtr buffer, int bufferSize);
    ...
    const int DUPLICATE_SAME_ACCESS = 0x0002;
    ...
    /// <summary>
    /// Find PIDs that hold an open handle to the given directory path (or any path under it).
    /// Times out after timeoutMs to avoid hanging indefinitely.
    /// </summary>
    public static List<int> FindDirectoryLockers(string targetDosPath, int timeoutMs = 8000) {
        var lockerPids = new HashSet<int>();
        int myPid = Process.GetCurrentProcess().Id;
        string targetSuffix = targetDosPath.Substring(2); // strip drive letter, keep "\path\..."

        var deadline = DateTime.UtcNow.AddMilliseconds(timeoutMs);

        // Enumerate all system handles
        int size = 0x200000; // start at 2MB
        ...
        try {
            ...
            do {
                ...
                if (status == STATUS_INFO_LENGTH_MISMATCH) {
                    ...
                    buffer = IntPtr.Zero;
                    size = needed + 0x100000;
                }
            } while (status == STATUS_INFO_LENGTH_MISMATCH);

            if (status != 0) return new List<int>(lockerPids);

            // First 2 ULONG_PTRs: NumberOfHandles, Reserved
            long handleCount = Marshal.ReadIntPtr(buffer).ToInt64();
            int entrySize = Marshal.SizeOf<SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX>();
            long headerSize = IntPtr.Size * 2;

            // Cache: process handle per PID
            ...
            try {
                for (long i = 0; i < handleCount; i++) {
                    ...
                    IntPtr entryPtr = IntPtr.Add(buffer, (int)(headerSize + i * entrySize));
                    ...
                    if (lockerPids.Contains(pid)) continue;

                    // Skip handles unlikely to be file/directory (heuristic: GrantedAccess == 0x0012019f
                    // is typical for directories, but too fragile. Just skip obviously non-file access masks.)
                    // We'll rely on the name query to filter.

                    if (!procHandles.TryGetValue(pid, out IntPtr procHandle)) {
                        procHandle = OpenProcess(PROCESS_DUP_HANDLE, false, pid);
                        procHandles[pid] = procHandle;
                    }
                    if (procHandle == IntPtr.Zero) continue;

                    IntPtr dupHandle = IntPtr.Zero;
                    if (!DuplicateHandle(procHandle, entry.HandleValue, GetCurrentProcess(),
                            out dupHandle, 0, false, DUPLICATE_SAME_ACCESS))
                        continue;

                    try {
                        string name = QueryObjectNameSafe(dupHandle);
                        if (name != null && name.EndsWith(targetSuffix, StringComparison.OrdinalIgnoreCase))
                            lockerPids.Add(pid);
                    } finally {
                ...
                foreach (var ph in procHandles.Values)
        ...
        return new List<int>(lockerPids);
    }

    /// <summary>
    /// Query object name with a timeout to avoid deadlocking on pipe/ALPC handles.
    /// </summary>
    static string QueryObjectNameSafe(IntPtr handle) {
        string result = null;
        var thread = new Thread(() => {
            ...
            try {
                int needed;
                int status = NtQueryObject(handle, ObjectNameInformation, buf, size, out needed);
                if (status == STATUS_INFO_LENGTH_MISMATCH && needed > 0) {
                    Marshal.FreeHGlobal(buf);
                    buf = Marshal.AllocHGlobal(needed);
                    status = NtQueryObject(handle, ObjectNameInformation, buf, needed, out needed);
                }
                if (status != 0) return;
                ushort len = (ushort)Marshal.ReadInt16(buf);
                if (len == 0) return;
                IntPtr strPtr = Marshal.ReadIntPtr(buf, IntPtr.Size);
                result = Marshal.PtrToStringUni(strPtr, len / 2);
            } finally {
                Marshal.FreeHGlobal(buf);
            }
        });
        ...
        thread.Start();
        if (!thread.Join(100)) return null; // timeout: skip this handle (likely pipe/ALPC)
        return result;
    }
}
'@ -ErrorAction SilentlyContinue

function Get-DirLockers([string]$dirPath) {
    $pids = [HandleScanner]::FindDirectoryLockers($dirPath)
    ...
    foreach ($pid in $pids) {
        try {
            ...
            $results.Add([pscustomobject]@{
                ...
                Critical = $false  # directory lockers are never kernel-critical
            })
        } catch { }
    }
    $results
